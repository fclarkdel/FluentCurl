...
void
curl_multi_socket_handle::add_handle(CURL* handle)
{
	throw_on_curl_multi_error(
		curl_multi_add_handle(_multi_handle, handle));
}
void
curl_multi_socket_handle::perform()
{
	throw_on_uv_error(
		uv_run(_event_loop, UV_RUN_DEFAULT));
...
void
curl_multi_socket_handle::throw_on_curl_multi_error(CURLMcode result)
{
	if(result != CURLM_OK)
		throw std::runtime_error(curl_multi_strerror(result));
}
int
curl_multi_socket_handle::throw_on_uv_error(int result)
{
	if(result >= 0)
		return result;
...
int
curl_multi_socket_handle::socket_cb(
	CURL* handle,
...
int
curl_multi_socket_handle::timer_cb(
	CURLM* handle,
	long timeout_ms,
	void* timer_data)
{
...
void
curl_multi_socket_handle::check_multi_info(CURLM* multi_handle)
{
		...
		curl_message = curl_multi_info_read(multi_handle, &running_handles))
	{
		if(curl_message == nullptr ||
		   curl_message->msg != CURLMSG_DONE)
			continue;

		if(curl_message->data.result != CURLE_OK)
			throw std::runtime_error(curl_easy_strerror(curl_message->data.result));
...
void
curl_multi_socket_handle::poll_event_cb(
	uv_poll_t* handle,
	int status,
	int events)
{
...
void
curl_multi_socket_handle::timeout_cb(uv_timer_t* handle)
{
...
void
curl_multi_socket_handle::poll_close_cb(uv_handle_t* handle)
{
	delete(PollContext*)handle->data;
	delete(uv_poll_t*)handle;
}
void
curl_multi_socket_handle::timer_close_cb(uv_handle_t* handle)
{
